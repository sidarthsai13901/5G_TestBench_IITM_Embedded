...
# import "customusbtools.py"
from customusbtools import UsbDeviceDescriptor, UsbTools
...
class Ftdi:
    ...
    def list_devices(cls, url: Optional[str] = None) -> \
        ...
        return UsbTools.list_devices(url or 'ftdi:///?',
                                     ...
                                     cls.DEFAULT_VENDOR)
    @classmethod
    def get_device_urls(cls, url: Optional[str] = None) -> List[str]:

        devdescs = UsbTools.list_devices(url or 'ftdi:///?',
                                        cls.VENDOR_IDS, cls.PRODUCT_IDS,
                                        cls.DEFAULT_VENDOR)
        urls = []
        for devdesc in devdescs:
            device_url = UsbTools.build_dev_url('ftdi', *devdesc)
            urls.append(device_url)
        return urls


    @classmethod
    def show_devices(cls, url: Optional[str] = None,
                     out: Optional[TextIO] = None) -> List[str]:
        ...
        devdescs = UsbTools.list_devices(url or 'ftdi:///?',
                                         cls.VENDOR_IDS, cls.PRODUCT_IDS,
                                         cls.DEFAULT_VENDOR)
        return UsbTools.show_devices('ftdi', cls.VENDOR_IDS, cls.PRODUCT_IDS,
                              devdescs, out)
