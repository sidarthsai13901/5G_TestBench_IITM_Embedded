...
int main() {
    ...
    DWORD numChannels = 0;
    unsigned char deviceAddress = 0x6C; // Replace with your I2C device's address
    unsigned char registerAddress = 0x0B; // Register you want to read
    ...
    if (status != FT_OK || bytesTransferred != 1) {
    ...
    status = I2C_DeviceRead(ftHandle, deviceAddress, sizeof(readData), readData, &bytesTransferred, I2C_TRANSFER_OPTIONS_START_BIT | I2C_TRANSFER_OPTIONS_STOP_BIT | I2C_TRANSFER_OPTIONS_NACK_LAST_BYTE);
    if (status != FT_OK) {
